...
- `ide/qlang-canvas.js` — `<qlang-canvas>` Web Component (Light DOM); adopts existing `<canvas>` child lub tworzy go lazily; API: `show(w, h, inputBuf)`, `blit(bitmap)`, `hide()`; render path jest jednolity: worker renderuje do własnego `OffscreenCanvas`, wysyła `ImageBitmap`, a komponent prezentuje go przez `ImageBitmapRenderingContext`; klawiatura nasłuchiwana na `window` (capture), mysz na samym canvasie
...
### Run mode i game canvas

- `main.js` zarządza Workerem run-mode oraz dwoma pętlami rAF: jedna odblokowuje `begin_frame` przez `STATE_EXT_WAIT`, druga prezentuje tylko ostatni otrzymany `ImageBitmap` na głównym wątku
- `wasm-runner.js` tworzy `SharedArrayBuffer` dla inputu gry w `gfx.init_window`, wysyła `{ type: 'window-open', w, h, inputBuf }`, a potem renderuje przez `WebGL2Renderer` do worker-owned `OffscreenCanvas`
- Aktywny render path nie transferuje DOM canvas do workera; bezpośredni handoff canvas został usunięty jako martwa ścieżka
- `btnCompile`, nowy `Run` albo `Stop` chowają canvas i kończą worker; ostatnia klatka nie jest utrzymywana po termination
...
## 13. Framework tworzenia komponentów IDE (Web Components)

Nowa funkcjonalność UI jest realizowana jako **mały, wyspecjalizowany Web Component** (Light DOM).
...
6. Dokumentacja    — ide.md §1: opis komponentu, API, eventy
```
